...
class QueryArchivedTransactionsResponse {
public:
    ...
    QueryArchivedTransactionsResponse() : code(0) {}
    ...
    // 输出运算符重载
    friend std::ostream& operator<<(std::ostream& os, const QueryArchivedTransactionsResponse& response) {
        os << "QueryArchivedTransactionsResponse{code=" << response.code 
    ...
    // JSON 序列化
    friend void from_json(const nlohmann::json& j, QueryArchivedTransactionsResponse& response) {
    ...
    friend void to_json(nlohmann::json& j, const QueryArchivedTransactionsResponse& response) {
        j = {
            {"code", response.code},
            {"message", response.message}
        };
